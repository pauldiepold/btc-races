...
const alreadyCanceled = ref(false)
const competition = ref<{ id: number; name: string } | null>(null)

// Automatische Abmeldung beim Laden der Seite
onMounted(async () => {
  if (!token) {
  ...
  try {
    ...
    >(`/api/registrations/cancel?token=${token}`)
    ...
    console.error('Fehler bei der Abmeldung:', e)
  } finally {
    loading.value = false
  }
})
...
<template>
  <BasePage heading="" back-link="/" back-link-text="Zurück zur Startseite">
    ...
    >
      <h2 class="text-center text-2xl font-bold">Abmeldung</h2>

      <div v-if="loading" class="py-8 text-center">
        <div
          class="mx-auto h-12 w-12 animate-spin rounded-full border-b-2 border-indigo-600"
        />
        <p class="text-dimmed mt-4 text-sm">Abmeldung wird verarbeitet...</p>
      </div>
      ...
      <div v-else-if="success" class="space-y-4">
        <UAlert
          ...
          :title="
            alreadyCanceled ? 'Bereits abgemeldet' : 'Abmeldung erfolgreich'
          "
          :description="
            alreadyCanceled
              ? 'Deine Anmeldung wurde bereits zu einem früheren Zeitpunkt storniert. Du bist nicht für den Wettkampf angemeldet.'
              : 'Deine Anmeldung wurde erfolgreich storniert. Du wirst nicht mehr für den Wettkampf angemeldet.'
          "
        />
