...
export default defineEventHandler(async (event: H3Event) => {
  ...
  try {
    ...
    const skippedRows: { row: number; name: string; email: string; error: string }[] = []
    const validMembers: any[] = []
    
    for (const [index, row] of data.entries()) {
      try {
        console.log(`Verarbeite Zeile ${index + 2}:`, JSON.stringify(row, null, 2))
    ...
    return {
      ...
      skipped: skippedRows.map(row => 
        `Zeile ${row.row}: ${row.name} (${row.email}) - ${row.error}`
      ),
      warnings: duplicateEmails.length > 0 
