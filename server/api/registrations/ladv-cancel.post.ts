...
export default defineEventHandler(async (event) => {
  try {
    ...
    // LADV-Abmeldung durchführen
    const success = await registrationsRepo.cancelFromLADV(
      parseInt(registrationId),
      coachName
    )
    if (!success) {
      return {
        error: {
          message: 'Fehler bei der LADV-Abmeldung',
          code: 'LADV_CANCELLATION_FAILED',
        },
        statusCode: 500,
      } as ApiResponse<null>
    }

    // Mail 1: LADV-Abmeldung durch Coach benachrichtigen
    try {
      const emailService = await NotificationEmailService.create(event)
      await emailService.sendLadvCancellationByCoach(parseInt(registrationId))
      console.log(
        `LADV-Abmeldungsmail gesendet für Registrierung ${registrationId}`
      )
    } catch (emailError) {
      console.error('Fehler beim Senden der LADV-Abmeldungsmail:', emailError)
    ...
    console.error('Unerwarteter Fehler bei der LADV-Abmeldung:', error)
    return {
      error: {
