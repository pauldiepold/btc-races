...
export const baseSchemas = {
  ...
  email: (fieldName: string) => ({
    required: () => z.string({
      required_error: messages.required(fieldName),
    }).min(1, messages.empty(fieldName)).email(messages.email),
    optional: () => z.string()
      .transform(val => val === '' ? undefined : val)
  ...
  url: (fieldName: string) => ({
    required: () => z.string({
      required_error: messages.required(fieldName),
    }).min(1, messages.empty(fieldName)).url(messages.url),
    optional: () => z.string()
      .transform(val => val === '' ? undefined : val)
  ...
  date: (fieldName: string) => ({
    required: () => z.string({
      required_error: messages.required(fieldName),
    }).min(1, messages.empty(fieldName)).regex(/^\d{4}-\d{2}-\d{2}$/, messages.date),
    optional: () => z.string()
      .transform(val => val === '' ? undefined : val)
      .pipe(z.string().regex(/^\d{4}-\d{2}-\d{2}$/, messages.date).optional()),
  }),
...
export async function validateSchema<T>(schema: z.ZodSchema, data: unknown): Promise<ValidationResult<T>> {
  try {
