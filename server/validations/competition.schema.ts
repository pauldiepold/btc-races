  ...
  if (data.registration_deadline && data.date) {
    ...
    if (deadlineDate > eventDate) {
      ctx.addIssue({
        ...
        message: 'Meldeschluss kann nicht nach dem Veranstaltungsdatum liegen',
      })
    }
  }
})
...
export function validateCompetition(data: unknown): Promise<ValidationResult<CompetitionOutput>> {
